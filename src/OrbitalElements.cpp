...
OrbitalElements::OrbitalElements( const std::vector<double>& JD, const std::vector< std::array<double,6> >& OE, const double& MU) : mu(MU), jds(JD), oes(OE) {
...
OrbitalElements::OrbitalElements(const std::string& fn, const double& MU) : mu(MU){
...
double OrbitalElements::trueAnomalyFromEccentricAnomaly( const double& EA, const double& eccentricity ){
    double beta = eccentricity/(1 + sqrt(1 - eccentricity*eccentricity));
    double s = sin(EA);
    return EA + 2*atan(beta*s/(1 - beta*sqrt(1 - s*s)));
}

double OrbitalElements::trueAnomalyFromMeanAnomaly( const double& MA, const double& eccentricity ){
    return trueAnomalyFromEccentricAnomaly(eccentricAnomalyFromMeanAnomaly(MA,eccentricity),eccentricity);
}

double OrbitalElements::trueAnomalyFromMeanAnomalyApprox( const double& MA, const double& eccentricity ){
...
double OrbitalElements::meanAnomalyFromTrueAnomalyApprox( const double& f, const double& eccentricity){
    double e2 = eccentricity*eccentricity;
...
double OrbitalElements::eccentricAnomalyFromMeanAnomaly( const double& MA, const double& eccentricity){
    double EA = MA;
...
double OrbitalElements::meanAnomalyFromEccentricAnomaly( const double& EA, const double& eccentricity) {
    return EA - eccentricity*sin(EA);
}

double OrbitalElements::eccentricAnomalyFromTrueAnomaly( const double& TA, const double& eccentricity ) {
    return atan2(sqrt(1 - eccentricity*eccentricity)*sin(TA), eccentricity + cos(TA));
}

double OrbitalElements::meanAnomalyFromTrueAnomaly( const double& f, const double& eccentricity){
    return meanAnomalyFromEccentricAnomaly(eccentricAnomalyFromTrueAnomaly(f,eccentricity),eccentricity);
}

std::array<double,6> OrbitalElements::kepler2cartesian(const std::array<double,6>& oe, const double& mu){   
...
std::array<double,3> OrbitalElements::kepler2position(const std::array<double,6>& oe){   
    double st = sin(oe[5]);
    double ct = cos(oe[5]);
...
std::array<double,6> OrbitalElements::cartesian2kepler(const std::array<double,6>& state, const double& mu) {
...
std::array<double,6> OrbitalElements::get(const double& jd) const {
...
std::array<double,6> OrbitalElements::getSimple(const double& jd) {
    if(jd > this->jds.back()){
        return this->oes.back();
    }
