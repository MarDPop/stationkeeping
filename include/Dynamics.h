...
public:    
    ...
    virtual std::array<double,N> get_state_rate(const std::array<double,N>& x, const double& t) const = 0;
...
public:
	...
	CR3BP() : mu(0.012155650403206974), mu1(0.987844349596793), mu_body1(3.986004418e5) , mu_body2(4.9048695e3) , sma(385000) , mean_motion(2.6590930417337446e-06)  {
	}

	CR3BP(const double& MU1, const double& MU2, const double& SMA) : mu(MU2/(MU1+MU2)), mu1(MU1/(MU1+MU2)), mu_body1(MU1) , mu_body2(MU2) , sma(SMA), mean_motion(sqrt((MU1 + MU2)/(sma*SMA*SMA))) {
	}
	...
	double getCL1(const double& yL,const int& n){
		...
		return 1/CUB(yL)* (mu + sign*mu1*std::pow(yL/(1 - yL),n+1));
	}

	double getCL2(const double& yL,const int& n){
		double sign = (n % 2 == 0) ? 1 : -1;
		return sign/CUB(yL)* (mu + mu1*std::pow(yL/(1 + yL),n+1));
	}
	
	double getCL3(const double& yL,const int& n){
		return 1/CUB(yL)* (mu1 + mu*std::pow(yL/(1 + yL),n+1));
	}
	...
	double getL2(){
		...
		for(int i = 0; i < 10;i++) {
			double f = L2 - mu1/SQ(L2 + mu) - mu/SQ(L2 - mu1);
			double df = 1 + 2*(mu1/CUB(L2 + mu) + mu/CUB(L2 - mu1));			
			L2 -= 0.7*f/df;
		}
		return L2;
	}
	...
	double getL3(){
		...
		for(int i = 0; i < 10;i++) {
			double f = L3 + mu1/SQ(L3 + mu) + mu/SQ(L3 - mu1);
			double df = 1 - 2*(mu1/CUB(L3 + mu) + mu/CUB(L3 - mu1));			
			L3 -= 0.7*f/df;
		}
		return L3;
	}
	
	std::array<double,6> getInitialState2(const double& Az0, const double& phi, const double& time, const bool& reverse = false, const bool& forward = true) {
		...
		double yLA = fabs(yL);
		double c2 = this->mu/CUB(yLA) + this->mu1/CUB(1 - yLA);
		...
		double n1 = sqrt(CUB(yLA)); // normalized, (might be a better way to compute this haha)
		double s = n1*time;
	...
	*/
	std::array<double,6> getHaloInitialState_3rd(const double& Az0, const double& phi, const double& time, const int& n = 1) {
...
public:	
	...
	EarthMoonSun(const double& JD0) : jd0(JD0) {
...
public:
	...
	Thruster(const double& isp, const double& thrust){
		this->thrust = thrust;
		this->v_exit = isp*9.806;
		this->mass_rate = thrust / this->v_exit;
	}
...
public:
	...
	virtual std::array<double,4> get(const std::array<double,7>& state, const double& time) {
		std::array<double,4> dState;
		
		return dState;
	}
};

class Dynamics3DOF : public Dynamics<7> {
	
	struct SSB_BODY {
		const OrbitalElements* oe_ssb;
		std::vector<std::array<double,3> > r_ref;
		std::vector<std::array<double,3> > dr_ref;
		
		SSB_BODY(const std::string& fn, const double& mu) : oe_ssb(new OrbitalElements(fn,mu)) {}
		
		void setFrom(std::vector<double> jds, const OrbitalElements* ref) {
			this->r_ref.clear();
			this->dr_ref.clear();
			for(double jd : jds) {
				const std::array<double,3>& body = OrbitalElements::kepler2position(ref->get(jd));
				const std::array<double,3>& ssb = OrbitalElements::kepler2position(oe_ssb->get(jd));
				std::array<double,3> r_emb = {ssb[0] - body[0],ssb[1] - body[1],ssb[2] - body[2]};
				this->r_ref.push_back(r_emb);
			}
			
			for(std::size_t i = 1; i < this->r_ref.size();i++){
				const std::array<double,3>& lo = this->r_ref[i-1];
				const std::array<double,3>& hi = this->r_ref[i];
				double djd = jds[i] - jds[i-1];
				std::array<double,3> dr_emb = {(hi[0] - lo[0])/djd,(hi[1] - lo[1])/djd,(hi[2] - lo[2])/djd};
				this->dr_ref.push_back(dr_emb);
			}
		}
	};
	
	OrbitalElements* Earth_EMB; // in EMB
	OrbitalElements* Moon_EMB;
	OrbitalElements* EMB;
	...
	std::vector<std::array<double,3> > d_moon;
	std::vector<std::array<double,3> > d_emb;
	std::vector<std::array<double,3> > d_omega;
	
	std::vector<SSB_BODY> bodies;

	double jd0,djd;
	
	double effective_area, reflectance;
...
public:
	...
	Dynamics3DOF(std::vector<std::string> files) {
		this->EMB = new OrbitalElements("resources/EMB_SSB.txt", OrbitalElements::ICRF_MU);
		this->Earth_EMB = new OrbitalElements("resources/EARTH_EMB.txt", OrbitalElements::EMBARY_MU);
		this->Moon_EMB = new OrbitalElements("resources/MOON_EMB.txt", OrbitalElements::EMBARY_MU);
		for(std::string body : files ){
			this->bodies.emplace_back(body,OrbitalElements::ICRF_MU);
		}
	}
	
	void setJD(const double& jd0,const double& jdf, const double& djd){
		this->jd0 = jd0;
		this->djd = djd;
		...
		for(SSB_BODY& body : bodies) {
			body.r_ref.clear();
			body.dr_ref.clear();
		}
		
		double jd = jd0;
		double jd_stop = jdf + djd;
		while(jd < jd_stop){
			this->jds.push_back(jd);
			this->r_earth.push_back(OrbitalElements::kepler2position(this->Earth_EMB->get(jd)));
			this->r_moon.push_back(OrbitalElements::kepler2position(this->Moon_EMB->get(jd)));
			
			std::array<double,6> emb = OrbitalElements::kepler2cartesian(this->EMB->get(jd),OrbitalElements::ICRF_MU);
			
			for(SSB_BODY& body : bodies) {
				const std::array<double,3>& pos = OrbitalElements::kepler2position(body.oe_ssb->get(jd));
				std::array<double,3> r_emb = {pos[0] - emb[0],pos[1] - emb[1],pos[2] - emb[2]};
				body.r_ref.push_back(r_emb);
			}
			
			// Get EMB Frame Info
			std::array<double,3> r = {emb[0],emb[1],emb[2]};
			this->r_emb.push_back(r);
			std::array<double,3> v = {emb[3],emb[4],emb[5]};
			std::array<double,3> h = Math::cross(r,v);
			double r2 = Math::dot(r,r);
			h[0] /= r2;
			h[1] /= r2;
			h[2] /= r2;
			this->omega.push_back(h);
		...
		int n = this->jds.size() - 1;
		for(int i = 0; i < n;i++) {
			const std::array<double,3>& lo = this->r_earth[i];
			const std::array<double,3>& hi = this->r_earth[i+1];
			std::array<double,3> dp = {(hi[0] - lo[0])/djd,(hi[1] - lo[1])/djd,(hi[2] - lo[2])/djd};
			this->d_earth.push_back(dp);
		}
		
		for(int i = 0; i < n;i++) {
			const std::array<double,3>& lo = this->r_moon[i];
			const std::array<double,3>& hi = this->r_moon[i+1];
			std::array<double,3> dp = {(hi[0] - lo[0])/djd,(hi[1] - lo[1])/djd,(hi[2] - lo[2])/djd};
			this->d_moon.push_back(dp);
		}
		
		for(int i = 0; i < n;i++) {
			const std::array<double,3>& lo = this->r_emb[i];
			const std::array<double,3>& hi = this->r_emb[i+1];
			std::array<double,3> dp = {(hi[0] - lo[0])/djd,(hi[1] - lo[1])/djd,(hi[2] - lo[2])/djd};
			this->d_emb.push_back(dp);
		}
		
		for(int i = 0; i < n;i++) {
			const std::array<double,3>& lo = this->omega[i];
			const std::array<double,3>& hi = this->omega[i+1];
			std::array<double,3> dp = {(hi[0] - lo[0])/djd,(hi[1] - lo[1])/djd,(hi[2] - lo[2])/djd};
			this->d_omega.push_back(dp);
		}
		
		for(SSB_BODY& body : bodies) {
			for(int i = 0; i < n;i++) {
				const std::array<double,3>& lo = body.r_ref[i];
				const std::array<double,3>& hi = body.r_ref[i+1];
				std::array<double,3> dp = {(hi[0] - lo[0])/djd,(hi[1] - lo[1])/djd,(hi[2] - lo[2])/djd};
				body.dr_ref.push_back(dp);
			}
		}
		
	}
	
	std::vector<double> getJDs(){
		return this->jds;
	}
	

	/*
	Matrix<7> getA(const std::array<double,7>& x, const double& jd, const double& delta) const {
		Matrix<7> A(0);
		A.set(0,3,1);
		A.set(1,4,1);
		A.set(2,5,1);
	...
	*/
	...
	std::array<double,3> getGravity(const std::array<double,7>& x, const double& jd) {
		...
		const std::array<double,3>& moon = this->r_moon[idx];
		const std::array<double,3>& emb = this->r_emb[idx];
		const std::array<double,3>& o = this->omega[idx];
		
		const std::array<double,3>& dE = this->d_earth[idx];
		const std::array<double,3>& dM = this->d_moon[idx];
		const std::array<double,3>& dEMB = this->d_emb[idx];
		const std::array<double,3>& dO = this->d_omega[idx];
		...
		for(SSB_BODY& body : this->bodies) {
			const std::array<double,3>& pos = body.r_ref[idx];
			const std::array<double,3>& delta = body.dr_ref[idx];
			for(std::size_t i = 0; i < 3; i++){
				g[i] += (body.oe_ssb->getMu()*(x[i] - (pos[i] + delta[i]*djd)));
			}
		}
		...
		a[0] = -g_E*rE[0] - g_M*rM[0] - g[0] - a_cen[0] - 2*a_cor[0];
		a[1] = -g_E*rE[1] - g_M*rM[1] - g[1] - a_cen[1] - 2*a_cor[1];
		a[2] = -g_E*rE[2] - g_M*rM[2] - g[2] - a_cen[2] - 2*a_cor[2];
		
		return a;
	}

	std::array<double,7> get_state_rate(const std::array<double,7>& x, const double& t) {
		double jd = this->jd0 + t/86400.0;
		
		std::array<double,3> a = getGravity(x, jd);
		
		std::array<double,4> u = this->control->get(x,t);
		
		std::array<double,7> dx = {x[3], x[4], x[5], a[0] + u[0], a[1] + u[1],a[2] + u[2],u[3]};
		return dx;
	}
};